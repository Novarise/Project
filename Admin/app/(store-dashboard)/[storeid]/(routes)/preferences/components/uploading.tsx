...
const UploadingPage: React.FC<ImageUploadProps> = ({
    ...
    const onUpload = async (ev: any) => {
        ...
        const response = await axios.post(`/api/${params.storeid}/billboards/uploads`, data);
    ...
    return (
        <div className="mb-4">
            <div className="mb-4 flex flex-wrap gap-2">
                {value[0] && (
                    <div key={value[0]} className='relative h-24 border-2  items-center justify-center rounded-lg'>
                        <img src={value[0]} alt="" className='max-h-[100%]'/>
                        <div className="absolute top-0 right-0 mb-2"
                            onClick={() => {onRemove(value[0]); setImage(''); value[0]=''}}
                        >
