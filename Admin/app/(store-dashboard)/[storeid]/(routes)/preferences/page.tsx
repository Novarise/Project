...
const Preferences: React.FC<PreferencesPage> = async ({ params }) => {
  ...
  const formattedProperties: PropertyColumn[] = allProperties?.properties.map(
    (property: any) => {
      ...
      const formattedValues = values.map((item: any) => {
        ...
        return {
          ...
          value,
          id: _id,
          createdAt: format(allProperties.createdAt, 'MMMM do, yyyy'),
        };
      });
