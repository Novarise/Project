...
import { createStripeCheckoutSession, getStripeCheckoutSession } from "@/services/stripeService";
...
export async function POST(request: Request, { params }: { params: { storeId: string } }) {
  ...
  for (let item of body.cartItems) {
    ...
    const detail = product.detail.find((d: any) => d._id.toString() === detailId);
    ...
    const orderItems = await Promise.all(body.cartItems.map(async (productData: { productId: string, detailId: string, quantity:number }) => {
    ...
    try {
      ...
      return Response.json({ BillboardData: body, url: stripeSession.url, success: true, sessionId: stripeSession.id });
    } catch (err) {
      console.log(err);
  ...
  export async function GET(request: NextRequest) {
    ...
    try {
      ...
      return Response.json({ BillboardData: {}, url: stripeSession.url, success: true });
    } catch (err) {
      console.log(err);
      return Response.json({ message: err, success: false });
    }
