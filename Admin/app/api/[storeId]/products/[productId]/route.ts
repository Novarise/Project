  ...
  try {
    ...
    if (ProductDoc.detail.length == 1) {
      ...
      return Response.json({
        ProductDeletedDoc: ProductDeletedDoc,
        success: true,
      });
    } else {
      ...
      return Response.json({
        ProductDeletedDoc: ProductDeletedDetailDoc,
        success: true,
      });
    }
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    if (flag) {
      ...
      const ProductUpdatedDoc = await Product.findByIdAndUpdate(
        filter,
        update,
        options,
      );
      ...
      return Response.json({
        ProductUpdatedDoc: ProductUpdatedDoc,
        success: true,
      });
    } else {
      const update = {
