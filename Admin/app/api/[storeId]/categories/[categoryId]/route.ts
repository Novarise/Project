...
export async function DELETE(request: Request, { params }: { params: { categoryId: string, storeId: string } }) {
    try {
        ...
        const CategoryDeletedDoc = await Category.deleteOne({ _id: params.categoryId })
        await Store.updateOne(
            { categories: params.categoryId },
            { $pull: { categories: params.categoryId } }
          );
        return Response.json({ CategoryDeletedDoc: CategoryDeletedDoc, success: true });
...
export async function PATCH(request: Request, { params }: { params: { categoryId: string, storeId: string } }) {
    ...
    try {
        ...
        const CategoryUpdatedDoc = await Category.findByIdAndUpdate(filter, update, options)

        return Response.json({ CategoryUpdatedDoc: CategoryUpdatedDoc, success: true });
    } catch (err) {
        console.log(err);
