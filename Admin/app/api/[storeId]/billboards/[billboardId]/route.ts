...
export async function PATCH(request: Request, { params }: { params: { billboardId: string, storeId: string } }) {
  ...
  try {
    ...
    const update = { $set: { label: label, imageUrl: imageUrl, active_billboard } };
    ...
    const BillboardUpdatedDoc = await Billboard.findOneAndUpdate(filter, update, options)
    ...
    if (active_billboard) {
      ...
      const ActiveBillboard = await Billboard.findOne(
        { storeId: params.storeId, userId: userId, active_billboard: true },)
    ...
    return Response.json({ BillboardUpdatedDoc: BillboardUpdatedDoc, success: true });
  } catch (err) {
    console.log(err);
...
export async function DELETE(request: Request, { params }: { params: { billboardId: string, storeId: string } }) {
  try {
    ...
    const BillboardDeletedDoc = await Billboard.deleteOne({ _id: params.billboardId })
    await Store.updateOne(
      { billboards: params.billboardId },
      { $pull: { billboards: params.billboardId } }
    );

    return Response.json({ BillboardDeletedDoc: BillboardDeletedDoc, success: true });
